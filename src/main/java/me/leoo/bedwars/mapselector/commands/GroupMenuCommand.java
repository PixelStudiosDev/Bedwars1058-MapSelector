...
public class GroupMenuCommand extends BukkitCommand {

    public GroupMenuCommand(String name) {
        super(name);
    ...
    public boolean execute(CommandSender sender, String string, String[] args) {
        ...
        if (args.length == 1) {
			...
			if (MapSelector.getPlugin().getBedwarsMode().equals(BedwarsMode.BEDWARSPROXY)) {
				...
				for (String group1 : group.split(",")) {
					if (groups.contains(group1)) {
						MapSelectorMenuProxy.openFirstGui(player, group1);
					} else {
            ...
            player.sendMessage(MapSelector.getPlugin().getMainConfig().getString("map_selector.menu.open.missing2"));
        }
        return false;
    }
