...
public class MapSelectorMenuBungee {
	...
	public static void OpenSelectorMenu(Player p, String group, int startMaps, int currentpage) {
		...
		com.andrei1058.bedwars.proxy.api.Language language = LanguageManager.get().getPlayerLanguage(p);
		arenas.sort(Comparator.comparing(a -> a.getDisplayName(language)));
		...
		for (String slotUsed : Config.config.getString("map_selector.menu.slots").split(",")) {
			...
			for (String s : Config.config.getList("map_selector.menu.item.map.lore")){
				mapLore.add(s.replace("{group_name}", StringUtil.firstLetterUpperCase(arenas.get(in).getArenaGroup())).replace("{available_games}", "5").replace("{times_joined}", String.valueOf(Yaml.getMapJoins(p, arenas.get(in).getArenaName()))).replace("{selections_type}", SelectorUtil.getSelectionsType(p)).replace("{remaining_uses}", SelectorUtil.getSelectionsType(p).equals(Config.config.getString("map_selector.selection.unlimited_message")) ? Config.config.getString("map_selector.selection.unlimited_message") : String.valueOf(Integer.parseInt(SelectorUtil.getSelectionsType(p)) - Main.database.getPlayerUses(p.getUniqueId()))).replace("{status}", arenas.get(in).getStatus().toString()).replace("{on}", String.valueOf(arenas.get(in).getCurrentPlayers())).replace("{max}", String.valueOf(arenas.get(in).getMaxPlayers())));
			}
