	...
	public static void OpenSelectorMenu(Player player, String group, int startMaps, int currentpage) {
		...
		for (String slotUsed : config.getString("map_selector.menu.slots").split(",")) {
			...
			for (String s : config.getList("map_selector.menu.item.map.lore")) {
				if (s.contains("{group_name}")) s = s.replace("{group_name}", SelectorUtil.firstLetterUpperCase(arenas.get(startMaps).getGroup()));
				...
				if (s.contains("{selections_type}")) s = s.replace("{selections_type}", SelectorUtil.firstLetterUpperCase(arenas.get(startMaps).getGroup()));
				...
				if (s.contains("{on}")) s = s.replace("{on}", String.valueOf(arenas.get(startMaps).getPlayers().size()));
